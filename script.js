...
document.querySelectorAll('.extra-btn').forEach(b=>{
  b.addEventListener('click', ()=>{
    ...
    showModal(service, `<p>You selected <strong>${service}</strong>. Please provide details in demo form to proceed.</p>
      ...
      <input id="m_contact" style="width:100%;padding:10px;border-radius:8px;border:1px solid #eee;margin-bottom:8px;">`, 'Book (Demo)', ()=>{
        const nm = document.getElementById('m_name')?.value || '';
        const ct = document.getElementById('m_contact')?.value || '';
        if(!nm || !ct) { alert('Please enter name and contact (demo).'); return; }
        alert(`Demo booking sent for ${service} — ${nm} • ${ct}`);
      });
  });
});
